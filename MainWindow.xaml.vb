...
Class MainWindow
    ...
    Public Shared config_path As String = "KBSTouchConfig.xml"
    Public Shared config_instant_path As String = "KBSTouchConfig_instant.xml"
    ...
    Public Shared position As Integer = 0
    ...
    Private ctrlPad As ControlPad
    Private clockWeather As ClockWeather
    ...
    Public Shared breakStartHour As UInteger = 21
    Public Shared breakEndHour As UInteger = 7
    Public Shared breakMode As Integer = BREAK_MODE.CLOCK
    Public Shared breakSchedule = False
    ...
    Public Shared clockStyle As Integer = CLOCK_STYLE.PAINT
    Public Shared weatherStyle As Integer = WEATHER_STYLE.PAINT
    ...
    Public Shared showcontrol As Boolean = False
    Public Shared noloop As Boolean = False
    Private minCfgVer As Version = New Version(2, 0, 0, 0)
    Public Shared screenIsOn As Boolean = True    '不能完全反映真实情况，因为鼠标激活屏幕无法获取
    Private showPathToggle As Boolean = True
    ...
    Public Enum BREAK_MODE As Integer
        SCROFF = 0
        CLOCK = 1
        NONE = 2
    End Enum

    Public Enum CLOCK_STYLE As Integer
        PAINT = 0
        CRYSTAL = 1
        PAGE = 2
        LCD = 3
    ...
    Public Enum WEATHER_STYLE As Integer
        PAINT = 0
        CARTOON = 1
    End Enum
...
#End If
        ...
        If Not LoadSettings() Then
            'MsgBox(Application.Current.Resources("msg_cfgerrexit"), MsgBoxStyle.Exclamation)
        ...
        ShowClock(True) '激活时钟，以便执行crontab
    End Sub
    ...
    Private Function LoadSettings() As Boolean
            ...
            Do
                ...
                Catch
                    If MsgBox(Application.Current.Resources("msg_loadcfgerr"), MsgBoxStyle.Question + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                        If New OptWindow().ShowDialog = False Then Return False
                    Else
                        Return False
                    End If
                End Try
        ...
        Do
            ...
            If config.Elements("BreakMode").Any Then breakMode = config.Element("BreakMode").Value
            If config.Elements("BreakStartHour").Any Then breakStartHour = config.Element("BreakStartHour").Value
            If config.Elements("BreakEndHour").Any Then breakEndHour = config.Element("BreakEndHour").Value
            ...
            If config.Elements("ClockStyle").Any Then clockStyle = config.Elements("ClockStyle").Value
            If config.Elements("WeatherStyle").Any Then weatherStyle = config.Elements("WeatherStyle").Value
    ...
    Private Sub pathThrd(pos As Integer, ByRef output As Integer)
#If DEBUG Then
        System.Diagnostics.Debug.Print(Format(Now, "hh:mm:ss") & " showPath: " & showPathToggle)
#End If
        ...
        If Not showPathToggle Then Exit Sub

        Dim crntpath = ListOfPic.Rows(pos - 1)("Path")
        Dim tbmove_sec = picmove_sec
        ...
        End If
        Dispatcher.Invoke(Sub()
                              tgt_tb = CType(mainGrid.FindName("tb_path" & mm), TextBlock)
                              tgt_tb.Text = crntpath.ToString
                              Dim anim_tbfadein As New Animation.DoubleAnimation(0, 0.7, New Duration(New TimeSpan(0, 0, 2)))
                              Dim anim_tbmovex As New Animation.DoubleAnimation(w / 10, w / 10 + RandomNum(h / 32, h / 25, True), New Duration(New TimeSpan(0, 0, tbmove_sec)))
                              Dim anim_tbmovey As New Animation.DoubleAnimation(h * 0.9, h * 0.9 + RandomNum(h / 32, h / 25, True), New Duration(New TimeSpan(0, 0, tbmove_sec)))
                              Animation.Timeline.SetDesiredFrameRate(anim_tbfadein, framerate)
                              Animation.Timeline.SetDesiredFrameRate(anim_tbmovex, framerate)
                              Animation.Timeline.SetDesiredFrameRate(anim_tbmovey, framerate)
                              Dim trans_trans As New TranslateTransform(w / 10, h * 0.9)
                              tgt_tb.RenderTransform = trans_trans
                              tgt_tb.BeginAnimation(TextBlock.OpacityProperty, anim_tbfadein)
                              trans_trans.BeginAnimation(TranslateTransform.XProperty, anim_tbmovex)
                              trans_trans.BeginAnimation(TranslateTransform.YProperty, anim_tbmovey)
    ...
    Private Sub mainThrd_KBE()
        ...
        Dispatcher.Invoke(Sub()
                              ...
                              Panel.SetZIndex(tb_path0, 4)
                              Panel.SetZIndex(tb_path1, 5)
                          End Sub)
        Dim tgt_img As Image
        Dim date_chkpoint As Integer = 1
        Dim text_chkpoint As Integer = 1
        Dim path_chkpoint As Integer = 1
        ...
        Do
            ...
            If position = path_chkpoint Then Task.Run(Sub() pathThrd(position, path_chkpoint))
    ...
    Private Sub mainThrd_Mix()
        ...
        Dim anim_fadein1, anim_fadein2 As Animation.DoubleAnimation
        Dim tgt_img As Image
        Dim date_chkpoint As Integer = 1
        Dim text_chkpoint As Integer = 1
        Dim path_chkpoint As Integer = 1
    ...
    Public Sub ShowClock(Optional ByVal forceInvisible = False) '默认非“强制不可见”
        If clockWeather Is Nothing Then clockWeather = New ClockWeather

        If forceInvisible Then
            clockWeather.Hide()
        Else
            If clockWeather.IsVisible Then
                clockWeather.Hide()
    ...
    Public Sub ScreenToggle(Optional ByVal TurnOff = True)
        If clockWeather Is Nothing Then clockWeather = New ClockWeather

        If TurnOff Then
            clockWeather.ScreenOff()
        Else
            clockWeather.ScreenOn()

        End If

    End Sub

    Private Sub LeftClick(sender As Object, e As MouseEventArgs)
        ShowControlPad()
        'ShowClock()
    End Sub

    Private Sub DoubleClick(sender As Object, e As MouseEventArgs)
        If showPathToggle Then
            showPathToggle = False
        Else
            showPathToggle = True
        End If
    End Sub
    Private Sub ShowControlPad()
        If ctrlPad Is Nothing Then
            ctrlPad = New ControlPad()
            ctrlPad.WindowStartupLocation = WindowStartupLocation.Manual
            ctrlPad.Left = (Me.Width - ctrlPad.Width) / 2
            ctrlPad.Top = Me.Height - ctrlPad.Height - 20

            'Dim wndHelper = New WindowInteropHelper(ctrlPad)
            'wndHelper.EnsureHandle()
        End If

        If ctrlPad.IsVisible Then
            Exit Sub
            'ctrlPad.Hide()
        Else
            ctrlPad.Show()
    ...
    Private Sub RightClick(sender As Object, e As MouseEventArgs)
        If Not ctrlPad Is Nothing Then ctrlPad.Hide()
    ...
    Private Sub Window_Closing(sender As Object, e As CancelEventArgs)
        Select Case nextcloseaction
            ...
            Case CloseAction.FadeToDesktop
                ...
                If ctrlPad IsNot Nothing Then ctrlPad.Close()
                If clockWeather IsNot Nothing Then clockWeather.Close()
